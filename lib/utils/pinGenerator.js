 ...
 * @param {number} length - Length of the PIN to generate
 * @returns {string} Random PIN code
 */
export const generateRandomPin = (length = 8) => {
  let result = ""
 ...
 * @param {number} length - Length of the PIN (excluding prefix)
 * @returns {Promise<string>} Unique PIN code
 */
export const generateUniquePin = async (prefix = "", length = 8) => {
  ...
  while (attempts < maxAttempts) {
    ...
    pin = prefix + generateRandomPin(length - prefix.length)
 ...
 * @param {number} length - Length of the PINs (excluding prefix)
 * @returns {Promise<string[]>} Array of unique PIN codes
 */
export const generateUniquePins = async (count, prefix = "", length = 8) => {
  ...
  while (pins.length < count && attemptsPerPin < maxAttemptsPerPin) {
    ...
    for (let i = 0; i < batchSize; i++) {
      candidates.push(prefix + generateRandomPin(length - prefix.length))
    }
