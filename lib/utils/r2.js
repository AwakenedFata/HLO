...
export const uploadToR2 = async (file, folder = "gallery") => {
  ...
  try {
    ...
    // Option 2: R2.dev subdomain (fallback)
    else if (process.env.R2_DEV_URL) {
      url = `${process.env.R2_DEV_URL}/${params.Key}`
    }
    // Option 3: Construct from account ID
    else {
      url = `https://pub-${process.env.R2_ACCOUNT_ID}.r2.dev/${params.Key}`
    }
