...
// POST upload image to S3 - FIXED VERSION
export async function POST(request) {
  try {
    ...
    try {
      ...
      return NextResponse.json(
        { error: "File validation failed", message: zodError.message },
        { status: 400 }
      );
    }
    ...
    logger.info(
      `Image uploaded to S3 by ${authResult.user.username}: ${uploadResult.key}`
    );
