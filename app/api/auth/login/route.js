...
import { loginSchema } from "@/lib/schemas/auth-schemas" // Import dari file auth-schemas
...
export async function POST(request) {
  try {
    ...
    // Validate request menggunakan loginSchema dari auth-schemas.js
    ...
    if (!user) {
      ...
      return NextResponse.json({ status: "error", message: "Username atau password salah" }, { status: 401 })
    }

    // PERBAIKAN: Periksa apakah method comparePassword ada
    if (typeof user.comparePassword !== "function") {
      logger.error(`comparePassword method not found on user model for username: ${username}`)
      return NextResponse.json(
        {
          status: "error",
    ...
    try {
      ...
      // PERBAIKAN: Simpan refresh token di database
      ...
      logger.error(`Error generating or saving tokens: ${tokenError.message}`)
      return NextResponse.json(
        {
          status: "error",
